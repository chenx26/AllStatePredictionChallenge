...
plot(cv05)
cv05_pred = predict(cv05, dtest, type = "response")
...
MLmetrics::MultiLogLoss(y_true = data_train_merged$last_event[-train_idx], y_pred = cv05_pred[,,1])

cv1=cv.glmnet(x = dtrain,
               y = data_train_merged$last_event[train_idx],
               family = 'multinomial', 
               alpha = 1, 
               parallel=TRUE, 
...
cv1_pred = predict(cv1, dtest, type = "response")

MLmetrics::MultiLogLoss(y_true = data_train_merged$last_event[-train_idx], y_pred = cv1_pred[,,1])

cv0=cv.glmnet(x = dtrain,
              y = data_train_merged$last_event[train_idx],
              family = 'multinomial', 
              alpha = 0, 
              parallel=TRUE, 
...
cv0_pred = predict(cv0, dtest, type = "response")
MLmetrics::MultiLogLoss(y_true = data_train_merged$last_event[-train_idx], y_pred = cv05_pred[,,1])
MLmetrics::MultiLogLoss(y_true = data_train_merged$last_event[-train_idx], y_pred = cv1_pred[,,1])
MLmetrics::MultiLogLoss(y_true = data_train_merged$last_event[-train_idx], y_pred = cv0_pred[,,1])


glmnet_cv_fit = cv.glmnet(x = train_data,
                          y = data_train_merged$last_event,
                          family = 'multinomial', 
                          alpha = 0.5, 
                          parallel=TRUE, 
                          nfolds = 10)
...
test_all.pred = predict(glmnet_cv_fit, dtest_all, type = "response")[,,1]
...
str(test_all.pred)
predictions = as.data.table(test_all.pred)
...
write.csv(predictions, file="./data/glmnet_no_mlr_first_last.csv", quote=TRUE, row.names=FALSE)
