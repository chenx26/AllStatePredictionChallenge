...
train$third_last_event = as.character(train$third_last_event)
...
test$third_last_event = as.character(test$third_last_event)
...
train_all$second_last_event = as.character(train_all$second_last_event)
...
test_all$third_last_event = as.character(test_all$third_last_event)
...
write.csv(predictions, file="./data/XGBoost_mlr_first_last_last2.csv", quote=TRUE, row.names=FALSE)
